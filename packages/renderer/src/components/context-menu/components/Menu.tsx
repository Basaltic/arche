...
import { TriggerEvent, MenuId, ContextMenuParams, MenuAnimation } from '../types';
...
import { cloneItems, getMousePosition, hasExitAnimation, isFn, isStr } from './utils';
...
function reducer(state: MenuState, payload: Partial<MenuState> | ((state: MenuState) => Partial<MenuState>)) {
  return isFn(payload) ? { ...state, ...payload(state) } : { ...state, ...payload };
}
...
export const Menu: React.FC<MenuProps> = ({
  ...
  function computeAnimationClasses() {
    ...
    if (isStr(animation)) {
      return cx({
        [`${STYLE.animationWillEnter}${animation}`]: animation && visible && !willLeave,
        [`${STYLE.animationWillLeave}${animation} ${STYLE.animationWillLeave}'disabled'`]: animation && visible && willLeave
      });
    } else if ('enter' in animation && 'exit' in animation) {
      return cx({
        [`${STYLE.animationWillEnter}${animation.enter}`]: animation.enter && visible && !willLeave,
        [`${STYLE.animationWillLeave}${animation.exit} ${STYLE.animationWillLeave}'disabled'`]: animation.exit && visible && willLeave
      });
    }
  ...
  const cssClasses = cx(STYLE.menu, className, { [`${STYLE.theme}${theme}`]: theme }, computeAnimationClasses());
  ...
  return (
    <MenuDataProvider {...propsFromTrigger}>
      <RefTrackerProvider refTracker={refTracker}>
        {visible && (
          <div {...rest} className={cssClasses} onAnimationEnd={handleAnimationEnd} style={menuStyle} ref={nodeRef} role="menu">
            {cloneItems(children, {
              propsFromTrigger,
              triggerEvent
            })}
